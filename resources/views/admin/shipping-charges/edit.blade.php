...
@section('content_header')
    <h1><i class="fas fa-edit"></i> Edit Shipping Charge</h1>
@stop
                    ...
                    <form action="{{ route('admin.shipping-charges.update', $shippingCharge->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select name="user_id" id="user_id" class="form-control @error('user_id') is-invalid @enderror" required>
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('user_id', $shippingCharge->user_id) == $client->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="item_name" id="item_name" 
                                   ...
                                   value="{{ old('item_name', $shippingCharge->item_name) }}" required>
                            @error('item_name')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="rate_per_unit" id="rate_per_unit" 
                                   ...
                                   value="{{ old('rate_per_unit', $shippingCharge->rate_per_unit) }}" step="0.01" min="0" required>
                            @error('rate_per_unit')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="unit_type" id="unit_type" class="form-control @error('unit_type') is-invalid @enderror" required>
                                <option value="qty" {{ old('unit_type', $shippingCharge->unit_type) == 'qty' ? 'selected' : '' }}>Quantity</option>
                                <option value="weight" {{ old('unit_type', $shippingCharge->unit_type) == 'weight' ? 'selected' : '' }}>Weight</option>
                            </select>
                            @error('unit_type')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="shipping_charge_per_kg" id="shipping_charge_per_kg" 
                                   ...
                                   value="{{ old('shipping_charge_per_kg', $shippingCharge->shipping_charge_per_kg) }}" step="0.01" min="0" required>
                        ...
                        <div class="form-group">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $shippingCharge->is_active) ? 'checked' : '' }}>
