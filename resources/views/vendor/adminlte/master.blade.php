...
<head>
    ...
    @php
        ...
        $customFavicon = $admin && $admin->favicon ? \Illuminate\Support\Facades\Storage::url($admin->favicon) : null;
    @endphp
    
    @if($customFavicon)
        {{-- Use custom favicon from admin profile --}}
    ...
    {{-- Extra Head Content (from config) --}}
    @if(config('adminlte.extra_head'))
        {!! config('adminlte.extra_head') !!}
    @endif
...
<body class="@yield('classes_body')" @yield('body_data')>
    ...
    @else
        @switch(config('adminlte.laravel_asset_bundling', false))
            @case('mix')
                <script src="{{ mix(config('adminlte.laravel_js_path', 'resources/js/app.js')) }}"></script>
            @break
